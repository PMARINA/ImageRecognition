...
# If your device has multiple camera interfaces, use this to select which one you would like to select
INPUTdEVICEnUMBER = 0
...
def contoursOnly(image, boundryLower, boundryUpper, num):
    # Use a gaussian blur to get rid of image noise, which will become annoying in the next step
...
# Debugging statement, as some computers may hang on the initialization part, or outright fail when attempting to use the camera, if none is attached
